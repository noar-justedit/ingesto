...
vm.runInContext(
  ...
  PAUSE_SRC + '\n' +
  extractFn('pauseGate') + '\n' +
  // 2.6.0: pauseGate announces "held" through the module's sendUI(), which
  // silently drops the message when there is no window — as here.
  'let __sent = 0; function sendUI(){ __sent++; }\n' +
  'globalThis.sentCount = () => __sent;\n' +
...
async function main() {
  ...
  // durations (a 500 s verify reported as 240 s, or clamped to zero).
  {
    ...
    const before = ctx.sentCount();
    ctx.setPause(true);
    const w4 = [ctx.pauseGate(win), ctx.pauseGate(win), ctx.pauseGate(win)];
    await new Promise(r => setTimeout(r, 300));
    ctx.setPause(false); ctx.pauseEnd();
    await Promise.all(w4);
    const held2 = ctx.sentCount() - before;
