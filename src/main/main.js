...
ipcMain.handle('recopy-failed', async (event, { sourcePath, sourceName, destPath, mode, proAlgo, proDoubleRead, files, destRelMap, destIndex, destName }) => {
  ...
  try {
    for (const f of (_copiedForSentinel || [])) {
      ...
      // Every generation: a second failure was set aside as "-2", "-3"…
      if (fs.existsSync(q)) { try { fs.unlinkSync(q); } catch (_) {} }
      for (let i = 2; i < 1000; i++) { const qi = `${q}-${i}`; if (!fs.existsSync(qi)) break; try { fs.unlinkSync(qi); } catch (_) {} }
    }
  } catch (_) {}
  return rest;
  } finally { activeCopyCount--; }
...
function holdsForeignContent(dir) {
  ...
  for (const e of entries) {
    if (e.name === '.DS_Store' || e.name.startsWith('._') || e.name === '.Spotlight-V100' ||
        e.name === '.fseventsd' || e.name === '.Trashes' || e.name === '.TemporaryItems' ||
        e.name === 'Thumbs.db' || e.name === 'desktop.ini') continue;
    if (e.isDirectory()) { if (holdsForeignContent(path.join(dir, e.name))) return true; continue; }
...
async function performCopyMulti(source, destinations, options, onProgress) {
  ...
  if (!options.fixedDestPath) {
    if (!folderSegs.length) {
      return abort('The folder template produced an empty name, which would write straight into the destination root. Ingest aborted — fix the template.');
    }
    ...
    if (!cleanSegment(resolveTemplateVars(templateLeaf(options.folderTemplate), source, tplNow))) {
      return abort('The last level of the folder template — the card\'s own folder — resolved to nothing, so the card would be written into the subfolder it shares with every other card. Ingest aborted — put something in the last level, or fill the fields it uses.');
    }
    ...
    for (const seg of folderSegs) {
      if (!isSafeFolderName(seg)) {
        return abort(`The folder template produced "${seg}" as a folder level, which is not a usable folder name. Ingest aborted — fix the template.`);
      }
    }
    if (folderSegs.length > MAX_FOLDER_DEPTH) {
      return abort(`The folder template asks for ${folderSegs.length} levels of subfolders (limit ${MAX_FOLDER_DEPTH}). Ingest aborted — remove a "/" from the template.`);
    }
    ...
    if (tplSegs.slice(0, -1).some(t => t.includes('{counter}'))) {
      return abort('The counter is in a subfolder level of the template. It must be part of the card\'s own folder — the last level, after the final "/". Ingest aborted.');
    }
  }
  ...
  for (const d of destinations) {
    ...
    if (!st || !st.isDirectory()) {
      return abort(`Destination "${d.name || root}" is not there any more (${root}). Nothing was written — reconnect the drive and start again.`);
    }
  }
  ...
  if (!options.fixedDestPath) {
    for (const d of destinations) {
      ...
      if (lst.isSymbolicLink() || !lst.isDirectory() || holdsForeignContent(cardDir)) {
        return abort(`The folder "${relFolder}" already exists on destination "${d.name || d.path}" and is not empty. INGESTO never writes into an existing card folder — that is how footage gets overwritten. Nothing was written. Use another counter, or check what is already in that folder.`);
      }
    }
  }
  ...
  if (onlyRel) {
    ...
    for (const rel of onlyRel) {
      ...
      for (const r of R) {
        r.errors++;
        r.errorList.push({ file: rel, error: 'this file is no longer on the card — it could not be re-copied', phase: 'scan' });
  ...
  for (const r of R) {
    ...
    if (r.active && !options.fixedDestPath) sweepPartFiles(r.destPath);
  }
  ...
  // already verified.
  for (const r of R) for (const tmp of r._tmps) { try { if (fs.existsSync(tmp)) fs.unlinkSync(tmp); } catch (_) {} }
      ...
      for (const r of group) {
        ...
        for (const c of r.copied) {
          ...
          try {
            ...
            try { fs.renameSync(c.dest, freeName(c.dest + FAILED_SUFFIX)); c._quarantined = true; }
            catch(_) {
...
ipcMain.handle('check-counter-collision', async (_, destPaths, counter, template) => {
  ...
  for (const destPath of (Array.isArray(destPaths) ? destPaths : [])) {
    walkDestination(destPath, matcher, template, (n, rel) => {
      if (wanted.has(n)) { hit = { counter: n, folder: rel }; return false; }
    });
    if (hit) return hit;
  }
  ...
  try {
    ...
    if (appendIfExists) {
      ...
      for (let i = 1; i < 100; i++) {
        ...
        if (!fs.existsSync(cand)) { writeFileAtomic(cand, text); return true; }
